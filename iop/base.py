...
def sign(secret,request,parameters):
    #===========================================================================
    # @param secret
    # @param parameters
    #===========================================================================
    sort_dict = sorted(parameters)
    api = request._api_pame
    protocol = request._protocol
    if(protocol == P_PROTOCOL_TOP):
        parameters_str = str().join('%s%s' % (key, parameters[key]) for key in sort_dict)
    elif(protocol == P_PROTOCOL_GOP):
...
class IopClient(object):
    ...
    def execute(self, request,access_token = None):
        ...
        sign_parameter[P_SIGN] = sign(self._app_secret,request,sign_parameter)

        protocol = request._protocol
        if(protocol == P_PROTOCOL_TOP):
            if(self._server_url[len(self._server_url)-1] == "/"):
                api_url = self._server_url + "sync"
            else:
                api_url = self._server_url + "/sync"
        elif(protocol == P_PROTOCOL_GOP):
            if(self._server_url[len(self._server_url)-1] == "/"):
                api_url = self._server_url + "rest"
            else:
                api_url = self._server_url + "/rest"
        else:
        ...
        try:
            if(request._http_method == 'POST' or len(request._file_params) != 0) :
                r = requests.post(api_url,sign_parameter,files=request._file_params, timeout=self._timeout, headers=request._header)
            else:
                r = requests.get(api_url,sign_parameter, timeout=self._timeout, headers=request._header)
